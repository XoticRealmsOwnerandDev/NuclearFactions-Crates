...
class Main extends PluginBase implements Listener{
	public function onEnable(){
		...
		$this->getLogger()->info("§aNuclearFactions Crates created by ItzCreeperBoy27 was loaded!");
	}
		...
		if($block->getId() === 54 || $block->getId() === 146) {
			if($block->getX() === $this->getConfig()->get("X") && $block->getY() === $this->getConfig()->get("Y") && $block->getZ() === $this->getConfig()->get("Z") && $level === $this->getConfig()->get("World")){
				if($heldItem->getId() === $keyid){
					...
					$player->sendMessage($prefix . "You opened the Crate and got rewards!");
				}else{
					$event->setCancelled();
					$player->sendMessage($prefix . "You need to be holding a CrateKey to get rewards!");
				}
			}
		}
	}
